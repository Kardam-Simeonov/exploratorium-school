<template>
  <div class="relative pt-72">
    <header class="relative px-12">
      ...
      <nuxt-img class="lg:block hidden 1xl:w-[48%] w-[55%] ml-auto absolute left-0 -right-1 bottom-[12%] object-cover object-center mask-header" src="/stock/pencils.jpg" />
    ...
    <section class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 1xl:gap-8 gap-6 mx-auto mt-16 mb-28 px-6 lg:max-w-[75rem] md:max-w-4xl max-w-lg">
      ...
      >
        <nuxt-img class="w-full aspect-[13/9] my-2 border-8 rounded-md rotate-6 hover:rotate-0 transition-transform duration-200 object-cover object-center" :src="runtimeConfig.public.strapiUrl + article.attributes.banner.data.attributes.url" />
