      ...
      <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-8 mx-auto px-12 2xl:max-w-7xl max-w-5xl">
        ...
        <article v-for="article, index in articles" :key="index" class="h-full w-full rounded-md rounded-br-3xl p-4 pb-6 bg-explo-darkpurple border-b-8 border-explo-lightblue shadow-md">
          ...
          <NuxtLink :to="article.attributes.slug">
            <button class="bg-explo-darkcard text-explo-whiteblue text-md rounded-md border-b-4 border-explo-darkercard hover:border-explo-darkgreen mt-4 p-2 shadow-lg">Научете Повече <fa icon="fa-solid fa-chevron-right " /></button>
          </NuxtLink>
        </article>
      </div>
    </section>
...
export default {
  ...
  methods: {
    async getArticles () {
      const result = await this.$axios.$get('http://localhost:1337/api/articles?fields=title%2C%20content%2C%20slug&populate=image')
